// hooks/use-library-movies.ts — simple, no EventSource
...
export function useLibraryMovies(urlQuery: string, filters: Filters) {
  ...
  // ── Main fetch — page 1 whenever query or filters change ─────────────────
  useEffect(() => {
    ...
    // Debounce text input — don't fetch on every keystroke
  ...
  const loadMore = useCallback(async () => {
    ...
    setLoadingMore(true)
    try {
      const data = await fetch(buildUrl(urlQuery, next, f)).then(r => r.json())
      setMovies(prev => {
