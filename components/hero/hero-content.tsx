    ...
    return (
        <AnimatePresence mode="sync">
            ...
            >
                ...
                {/* Title */}
                <motion.div variants={itemVariants} className="text-3xl md:text-5xl lg:text-7xl font-title  text-white tracking-wide max-w-4xl mb-2! text-balance uppercase">
                    {movie.title}
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="flex flex-wrap items-center gap-2! md:gap-2! mb-2!">
                    ...
                    <Badge variant="link" className=" text-text-muted text-xs gap-1 rounded-md">
                        {movie.vote_average.toFixed(1)} ({formatVotes(movie.vote_count || 0)})
                    </Badge>
